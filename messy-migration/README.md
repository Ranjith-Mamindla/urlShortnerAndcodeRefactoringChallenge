...
-->Split monolithic code into modular files (e.g., app.py, routes/user_routes.py, schemas.py, database.py) to improve separation of concerns and maintainability.

-->Used Flask Blueprints for user-related routes to keep the API scalable and organized.

-->Encapsulated database connection logic into a dedicated module to avoid global connection usage.

2.Security Improvements

-->Replaced all raw SQL string interpolation with parameterized queries to prevent SQL injection attacks.

-->Validated and parsed incoming JSON payloads using Pydantic models, enforcing proper data types and mandatory fields.

-->Passwords are still stored in plain text (due to assignment scope), but structured to allow easy integration of password hashing in the future.

3.Best Practices

-->Added appropriate HTTP status codes (e.g., 200 OK, 201 Created, 400 Bad Request, 404 Not Found) in API responses.

-->Improved error handling with clear messages for client errors and database errors.

-->Used JSON responses instead of plain strings to standardize API output.

-->Added minimal tests for critical endpoints (e.g., create user, login) to ensure basic functionality.

-->Configured Flask app with debug=False for production readiness, while enabling debug during development.

4.Documentation & Usability

-->Added clear README explaining setup, usage and changes made.

-->Provided sample curl and PowerShell commands for testing endpoints.

-->Used Python's venv and requirements.txt for dependency management.
